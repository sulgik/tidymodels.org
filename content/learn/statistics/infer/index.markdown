...
p-값이 0.042 으로 꽤 작은 것 같습니다 -- 실제 주당 평균 근무시간이 40 이라면, 우리 샘플 평균이 40 에서 이 만큼(1.382 시간) 떨어져 있을 확률이 0.042 입니다. 이는 여러분이 분석 *전*에 결정한 유의수준 `\(\alpha\)` 에 따라 통계적으로 유의하게 다를 수도 있도 아닐 수도 있습니다. `\(\alpha = .05\)` 로 설정했다면, 관측한 차이는 통계적으로 유의하겠지만, `\(\alpha = .01\)` 로 설정했다면, 그렇지 않을 것입니다.
