...
pkgs <- c("tidymodels", "ggiraph", "embed", "AmesHousing")
...
tf_embed <- 
  ...
  step_embed(
    ...
    num_terms = 5, 
    hidden_units = 10, 
    options = embed_control(epochs = 150, validation_split = 0.2)
  ) %>% 
  prep(training = ames)
...
hood_coef <- 
  ...
  # Make names smaller
  rename_at(vars(contains("emb")), .funs = ~ gsub("Neighborhood_", "", .x, fixed = TRUE))
hood_coef
