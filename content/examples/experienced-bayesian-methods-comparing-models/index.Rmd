...
tags: [tune, rsample, parsnip, tidyposterior]
categories: [model comparisons, tuning]
---
...
pkgs <- c("modeldata", "tidyposterior", "tidymodels", "rpart", "kernlab", "earth")
...
This article describes how resampled models can be formally compared using _Bayesian analysis_ (in _the spirit_ of [Benavoli _et al_ (2017)](http://people.idsia.ch/~marco/papers/2017jmlr-tests.pdf)). 
. It does assume some basic knowledge of statistics and Bayesian modeling. 
...
svm_tune <- 
  svm_mod %>% 
  tune_grid(compressive_strength ~ ., resamples = folds, grid = 20)
...
mars_tune <- 
  mars_mod %>% 
  tune_grid(compressive_strength ~ ., resamples = folds, grid = mars_grid)
...
extract_rmse <- function(x) {
  ...
  collect_metrics(x, summarize = FALSE) %>% 
    ...
    inner_join(select_best(x, metric = "rmse",)) %>% 
...
rmse_values %>% slice(1:5)

# Merge these values back into the original rsample object.
folds <- 
  folds %>% 
  arrange(id, id2) %>% 
  bind_cols(rmse_values %>% dplyr::select(-id, -id2))
```
...
To fit this model, the `family` argument to `stan_glmer()` can be passed in. The default link is the inverse and no extra transformation will be used. The `tidyposterior::perf_mod()` function below uses  `rstanarm::stan_glmer()`, which produces _a lot_ of output. 
...
The `tidy()` method saves a tibble with columns for the model and draws of the posterior distributions. The subsequent `summary()` method returns a tibble with means and tail quantiles of each posterior. 

```{r gamma-stats}
gamma_post <- tidy(gamma_model, seed = 3750)
gamma_mean <- summary(gamma_post, prob = 0.9)
gamma_mean
```
...
```{r linear-linear-fit}
linear_model <- perf_mod(folds, seed = 74, iter = 2000, chains = cores, cores = cores)
```
```{r linear-linear}
...
```{r si, echo = FALSE}
small_session(c(pkgs, "rstan", "rstanarm"))
```
