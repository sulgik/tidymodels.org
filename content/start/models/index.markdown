...
[Constable (1993)](https://link.springer.com/article/10.1007/BF00349318) 데이터에서 사육법에 따른 바다성게의 크기 차이를 살펴봅시다. 실험 시작점에서의 성게의 초기 크기가 아마도 얼마나 클 수 있는지에 대해 영향을 줄 것입니다.
...
이러한 데이터셋에 two way 분산분석 ([ANOVA](https://www.itl.nist.gov/div898/handbook/prc/section4/prc43.htm)) 모델을 사용할 수 있는데, 연속형 설명변수와 명목형 설명변수가 있기 때문입니다. 직선의 기울기가 적어도 두 개 이상의 사육법에 대해 달라 보이기 때문에, two-way interaction 을 가진 모델을 만들어 봅시다. 다음과 같이 R 공식을 선언합니다.
...
initial volume 에 따라 width 가 변하는 회귀 모형입니다. 사육법에 따라 다른 기울기와 절편을 갖게 됩니다.

이러한 모델에 대해, 첫단계로 ordinary least squares 를 시도해보면 좋습니다. tidymodels 에서 원하는 모델의 _함수포맷_ 을 [parsnip 패키지](https://tidymodels.github.io/parsnip/)를 사용하여 명시합니다. 우리 모델은 출력값이 수치형이고, 모델이 기울기와 절편에 대해 선형입니다. 이러한 모델타잎은 ["linear regression"](https://tidymodels.github.io/parsnip/reference/linear_reg.html) 입니다. 다음과 같이 선언합니다: 
...
엔진의 값은 모델을 훈련/적합에 사용되는 소프트웨어와 추정방법의 결합(mash-up)인 경우가 많습니다. 예를 들어, 엔진을 `lm` 으로 두어 ordinary least squares 를 사용합니다:
...
사용할 수 있는 엔진목록은 [`linear_reg() 문서`](https://tidymodels.github.io/parsnip/reference/linear_reg.html) 에 있습니다. 모델 객체를 `lm_mod` 이름으로 저장합니다:
...
이제 [`fit()`](https://tidymodels.github.io/parsnip/reference/fit.html) 함수로 모형을 추정/훈련합니다:
...
모델 파라미터 추정값과 통계적 특징값을 추출해 봅시다. `summary()` 함수를 `lm` 객체에 사용할 수 있지만, 출력되는 형태가 깔끔하지 않습니다. 많은 모델에 사용할 수 있는 `tidy()` 함수는 결과를, 예측한대로 그리고 유용한 형태로 요약하여 제공합니다 (예: 표준 열 이름을 가진 데이터프레임):
...
객체 `lm_fit` 에도 빌트인된 `lm` 모델 출력이 있어, `lm_fit$fit` 으로 접근할 수 있지만, 적합된 parsnip 모델 객체에는 예측에 있어, 장점이 있습니다. 

예를 들어, 출판을 위해 20ml 의 초기부피로 실험을 시작한 성게들의 평균 성체크기의 플롯을 그리고 싶을 수 있습니다. 이러한 그래프를 그리기 위해, 예측할 대상인 새로운 데이터로 시작합니다. 
...
`predict()` 함수를 사용하여 20ml 에서 평균값을 계산하여, 예측 결과를 얻을 수 있습니다.

변동성에 대해 잘 전달하는 것도 중요하기 때문에, 예측 신뢰구간을 구할 필요가 있습니다. `lm()` 으로 모델을 직접 적합했다면, `predict.lm()` 의 [문서 페이지](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/predict.lm.html)에 몇 분을 투자해서, 신뢰구간을 구할 수 있을 것입니다. 하지만, 성게 크기를 예측하기 위해 다른 모형을 사용하기를 결정했다면 ( _스포일러:_ 예정됨), 완전히 다른 문법이 필요할 것입니다.

tidymodels 에서는 예측값들의 타잎이 표준화되기 때문에 동일한 문법을 사용하여 평균과 신뢰구간을 구할 수 있습니다.
...
tidymodels 컨벤션으로 인해 예측값티블의 열이름은 항상 표준화된 이름을 가지게 됩니다. 그 결과로 원데이터와 예측값을 재사용할 수 있는 포맷으로 조합하기 쉬워집니다:
...
팀원 대부분이 플롯에 만족 _했지만_ [베이지언 분석](https://bayesian.org/what-is-bayesian-analysis/)에 관한 첫번째 책을 읽은 한사람은 그렇지 않았습니다. 모델이 베이지언 방법으로 추정했다면 결과가 달랐을지에 관해 알고 싶어합니다. 이러한 분석에서 [_prior distribution_](https://towardsdatascience.com/introduction-to-bayesian-linear-regression-e66e60791ea7)이 각 모델 파라미터에 관해 파라미터로 가능한 값들이 (관측 데이터에 노출되기 전에) 선언되어야 합니다. 
논의 끝에, 이 그룹은 prior 가 종모양이지만, 값범위가 어떻게 되어야 하는지에 관한 아이디어가 아무도 없었기 때문에, 보수적인 방법을 취해서, 코시 분포 (자유도 1인 t-분포와 동일) 를 사용하여 prior 를 _넓게_ 만들기로 합니다.

[rstarnarm 패키지문서](https://mc-stan.org/rstanarm/articles/priors.html)에는 이 모델을 추정하는 데 `stan_glm()` 함수를 사용할 수 있고, 인수이름은 `prior` 와 `prior_intercept` 라고 적혀 있습니다. 
`linear_reg()` 에는 stan 엔진이 있다는 것을 알게 되었습니다. 
이러한 사전분포 인수들은 Stan 소프트웨어에 특화되기 때문에, [`parsnip::set_engine()`](https://tidymodels.github.io/parsnip/reference/set_engine.html) 의 인수형태로 전달됩니다.
이후에는, 앞에서와 완전히 같은 `fit()` 호출을 사용합니다:
...
베이지언 분석의 적합과정 안에는 랜덤숫자를 생성하는 것이 포함되어 있습니다. `set.seed()` 를 사용하여 이 코드를 실행할 때마다 같은 (pseudo-)랜덤 숫자가 생성되도록 할 수 있습니다. 숫자 `123` 은 특별한 의미가 있거나 우리 데이터와 관련이 있는 것은 아닙니다; 단지 랜덤 숫자를 선택할 때 사용된 "시드" 입니다.

파라미터 표를 새로 얻기 위해, `tidy()` 방법을 다시 한번 사용할 수 있습니다:
...
`linear_reg()` 와 같은 함수를 사용하여 모델을 정의하는 단계를 추가하는 것은 필요 없을 것 같은데 `lm()` 을 호출하는 것이 훨씬 간단해 보이기 때문입니다. 하지만, 표준 모델링 함수들의 문제는, 실행하는 것에서 하고 싶은 것을 분리하지 못한다는 것입니다. 예를 들어, 공식을 실행하는 프로세스는 공식이 바뀌지 않았을 때도 모델 호출들을 따라 반복적으로 일어나야 합니다; 이 계산을 재사용할 수 없습니다.
